...
from src.utils import FEATURE_COLUMNS, OUTPUT_DIR, ensure_directories, load_json, load_signals, top_feature_explanation
...
def score_dataframe(df: pd.DataFrame, baseline: dict) -> pd.DataFrame:
    ...
    for column in FEATURE_COLUMNS:
        ...
        robust_z = (scored[column] - median) / (1.4826 * mad)
    ...
    scored["alert_flag"] = (scored["risk_score"] >= baseline["threshold"]).astype(int)
